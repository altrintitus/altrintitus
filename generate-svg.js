...
// -----------------------------------------------------------
// UPDATED WIDE AND UNCLUTTERED LEFT HUD
// -----------------------------------------------------------
function svgHUDLeft(stats,P){
  const x=16,y=H-235,w=280,h=220;
  ...
  s+=`<text x="${x+16}" y="${cy}" font-family="monospace" font-size="8.5" font-weight="bold" fill="rgba(100,170,255,0.6)" letter-spacing="1.5">TOP REPOS</text>`;cy+=14;
  P.slice(0,5).forEach((p,i)=>{
    const c=PCOL[i];
    s+=`<circle cx="${x+22}" cy="${cy+3}" r="4" fill="${c.f}"/>`;
    s+=`<text x="${x+34}" y="${cy+7}" font-family="monospace" font-size="8.5" fill="rgba(200,220,255,0.7)">${esc(p.name.slice(0,16))}</text>`;
    s+=`<text x="${x+w-16}" y="${cy+7}" text-anchor="end" font-family="monospace" font-size="8.5" fill="rgba(200,220,255,0.4)">${p.count}</text>`;
    cy+=15;
  });
  return s;
}
...
// -----------------------------------------------------------
// UPDATED WIDE AND UNCLUTTERED RIGHT TECH STACK
// -----------------------------------------------------------
function svgTechStack(tech){
  ...
  Object.entries(tech).forEach(([cat,items])=>{
    ...
    let bx=px+14,tempCy=cy+26; 
    let calcBx=px+14,calcCy=26;
    
    items.forEach(item=>{
      const tw=item.length*6+16;
      if(calcBx+tw>px+pw-14){calcBx=px+14;calcCy+=24;} 
      calcBx+=tw+6;
    });
    
    const cardH=calcCy+14;
    ...
    items.forEach(item=>{
      const tw=item.length*6+16;
      if(bx+tw>px+pw-14){bx=px+14;tempCy+=24;}
      s+=`<rect x="${bx}" y="${tempCy}" width="${tw}" height="19" rx="5" fill="rgba(60,120,220,0.07)" stroke="rgba(80,140,255,0.18)" stroke-width="0.5"/>`;
      s+=`<text x="${bx+tw/2}" y="${tempCy+13}" text-anchor="middle" font-family="monospace" font-size="8" fill="rgba(180,210,255,0.88)">${esc(item)}</text>`;
      bx+=tw+6; 
    });
